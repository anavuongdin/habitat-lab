...
class SRNN(nn.Module):
    ...
    def forward(self, robot_node, temporal_edges, spatial_edges, rnn_hxs, masks, infer=False):
        if infer:
            # Test time
            seq_length = 1
            nenv = self.nenv
        ...
        robot_node = reshapeT(robot_node, seq_length, nenv)
        temporal_edges = reshapeT(temporal_edges, seq_length, nenv)
        spatial_edges = reshapeT(spatial_edges, seq_length, nenv)
...
if __name__ == '__main__':
    config = Config()
    model = SRNN(None, config)
    x = torch.rand((180, 1, 7))
    y = torch.rand((180, 1, 2))
    z = torch.rand((180, 5, 2))
    inputs = dict()
    inputs['robot_node'] = x
    inputs['temporal_edges'] = y
    inputs['spatial_edges'] = z
    rnn_hxs = dict()
    rnn_hxs['human_node_rnn'] = torch.rand((6, 1, 128))
    rnn_hxs['human_human_edge_rnn'] = torch.rand((6, 6, 256))
    masks = torch.rand((180, 1))

    print(model(inputs, rnn_hxs, masks))
